...
2023.4.5 更新说明
修改噩梦的怪物血量，从每级增加100%下调到每级增加50%
咦，最近没BUG了
原来作者是最大的bug

2023.3.7 更新说明
修复噩梦金牙的蜡烛击晕时间被错误的配置为5000秒的问题
修复挂机状态下噩梦结算层数发生变化的问题
修正噩梦卡卡列夫和卡罗奖励物品等级不正确的问题

2023.2.22 更新说明
红包掉落装备不再受当前地图等级影响，仅受玩家等级影响，70级时固定掉落250级装备。
修复地图面板有时候活跃地图文字不显示的问题。
修复无尽副本会多余加入一次（但没有奖励）的问题
修复普通副本可以持续加入（但没有奖励）的问题

2023.2.17 更新说明
添加无尽副本机制，原有噩梦钥石转换成无尽副本钥石。
更新后首次登录可能部分队列中无尽副本无法收益，属于正常现象，重新进入副本即可正常收益。
通过无尽副本，装等可以突破300级。
对应移除了拾取规则的等级上限
...
解决面板吸血显示数值不正确的bug
修改物品排序的规则，现在同品质的装备会按照装备位置和等级进行排序。
现在年兽副本不会对未通过白银试炼的角色显示。
修复了闪电风暴吸收伤害类型错误的问题
修复了部分元素怪物没有完全吸收指定伤害类型的问题
修复了法师的龙息术和霜之新星可以对免疫该类型的元素怪产生效果的问题。
